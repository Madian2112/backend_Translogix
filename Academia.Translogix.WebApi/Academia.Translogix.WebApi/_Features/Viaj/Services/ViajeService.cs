...
namespace Academia.Translogix.WebApi._Features.Viaj.Services
{
    public class ViajeService
    {
        ...
        private readonly OpenRouteServicer _openRouteService;
        private readonly IMapper _mapper;
        private readonly ViajeDominioService _viajeDominioService;
        private readonly CommonService _commonService;

        public ViajeService(OpenRouteServicer openRouteService, UnitOfWorkBuilder unitOfWork,
        ...
        public Usuarios usarioExistente(int usuario_id)
        {
            ...
            return (Usuarios)_unitOfWork.Repository<Usuarios>()
                    .AsQueryable()
                    .AsNoTracking()
        ...
        {
            List<ViajesDto> data = new List<ViajesDto>();
            try
            {
            ...
            {
                var response = ApiResponseHelper.ErrorDto(data, Mensajes._15_Error_Operacion + ex.Message);
        ...
        {
            List<RutaAgrupadaResponse> data = new List<RutaAgrupadaResponse>();
            try
            {
                ...
                if (!noNulos.Success)
                    return ApiResponseHelper.ErrorDto(data, $"{Mensajes._06_Valores_Nulos}{noNulos.Message}");

                var usuarioEntidad = usarioExistente(request.usuario_creacion);


                if (!usuarioEntidad.es_admin)
                    return ApiResponseHelper.ErrorDto(data, Mensajes._25_Usuario_Administrador);

                bool esNull = _commonService.EntidadExistente<Usuarios>(request.usuario_creacion);

                if (!esNull)
                    return ApiResponseHelper.ErrorDto(data, Mensajes._24_Usuario_No_Encontrado);
                ...
                if (ubicaciones == null || ubicaciones.Count < 1)
                {
                    return ApiResponseHelper.ErrorDto(data, Mensajes._26_Ubicacion_Requerida);
                }
                ...
                foreach (var viaje in result.RutasAgrupadas)
                {
                    ...
                    if (!resultDomainViajes.Success)
                        return ApiResponseHelper.ErrorDto(data, resultDomainViajes.Message);
                    ...
                    var entidadesViajeDetalles = _mapper.Map<List<ViajesDetalles>>(modeloViajeDetalles);

                    _unitOfWork.Repository<ViajesDetalles>().AddRange(entidadesViajeDetalles);
            ...
            {
                return ApiResponseHelper.ErrorDto(data, $"{Mensajes._15_Error_Operacion}{ex.Message}");
            }
        }
