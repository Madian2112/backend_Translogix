...
namespace Academia.Translogix.WebApi._Features.Gral.Services
{
    public class EstadoCivilService
    {
        ...
        public ApiResponse<List<EstadosCivilesDto>> ObtenerTodos()
        {
            List<EstadosCivilesDto> data = new List<EstadosCivilesDto>();
            try
            {
                var lista = _unitOfWork.Repository<EstadosCiviles>().AsQueryable().AsNoTracking().ToList();
                var listaDto = _mapper.Map<List<EstadosCivilesDto>>(lista);
            ...
            catch (Exception ex)
            {
                ...
                var response = new ApiResponse<List<EstadosCivilesDto>>(false, errorMessage, data, statusCode);
