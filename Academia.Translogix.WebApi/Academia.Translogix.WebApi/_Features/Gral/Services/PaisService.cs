...
namespace Academia.Translogix.WebApi._Features.Gral.Services
{
    public class PaisService
    {
        ...
        public ApiResponse<List<PaisesDto>> ObtenerTodos()
        {
            List<PaisesDto> data = new List<PaisesDto>();
            try
            {
                var lista = _unitOfWork.Repository<Paises>().AsQueryable().AsNoTracking().ToList();
                var listaDto = _mapper.Map<List<PaisesDto>>(lista);
            ...
            catch (Exception ex)
            {
                ...
                var response = new ApiResponse<List<PaisesDto>>(false, errorMessage, data, statusCode);
