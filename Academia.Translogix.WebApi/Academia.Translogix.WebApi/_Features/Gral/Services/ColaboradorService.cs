...
namespace Academia.Translogix.WebApi._Features.Gral.Services
{
    public class ColaboradorService
    {
        ...
        {
            List<ColaboradoresDto> data = new List<ColaboradoresDto>();
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                var response = new ApiResponse<List<ColaboradoresDto>>(false, errorMessage, data, statusCode);
        ...
        public ApiResponse<string> InsertarColaboradorPersona(ColaboradoresDtoInsertar modeloInsertar)
        {
            ...
            try
            {
                ...
                bool estadoCivilExistente = _commonService.EntidadExistente<EstadosCiviles>(mappColaborador.estado_civil_id);
        ...
        {
            List<ColaboradoresSinViaje> data = new List<ColaboradoresSinViaje>();
            try
            {
                if (!_commonService.EntidadExistente<Sucursales>(sucursal_id))
                    return ApiResponseHelper.ErrorDto(data, Mensajes._22_Sucursal_No_Encontrada);

                var query = (from co in _unitOfWork.Repository<Colaboradores>().AsQueryable()
                             join sucu in _unitOfWork.Repository<SucursalesColaboradores>().AsQueryable() on co.colaborador_id equals sucu.colaborador_id
                             ...
                             && !_unitOfWork.Repository<Viajes>().AsQueryable()
                                 .Join(_unitOfWork.Repository<ViajesDetalles>().AsQueryable(),
                                       v => v.viaje_id,
                                       viaj => viaj.viaje_id,
                                       (v, viaj) => new { v, viaj })
                ...
                return ApiResponseHelper.Success(query, Mensajes._02_Registros_Obtenidos);
            }
